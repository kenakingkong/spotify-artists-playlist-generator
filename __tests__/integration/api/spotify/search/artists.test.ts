...
import handler from "@/pages/api/spotify/search/artists";
...
describe("/api/spotify/search/artists", () => {
  const MOCK_ARTIST = {
    type: "artist",
    name: "test",
    uri: "test",
  ...
  it("returns 200 and Spotify artist data", async () => {
    const fakeData = {
      artists: { items: [MOCK_ARTIST] },
    };

    mockedAxios.get.mockResolvedValueOnce({ data: fakeData });

    const { req, res } = createMockApi();
    req.query = { q: "jigitz", limit: "1" };
